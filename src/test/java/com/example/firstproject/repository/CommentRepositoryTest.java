...
class CommentRepositoryTest {
    ...
    @Test
    @DisplayName("특정 게시글의 모든 댓글 조회") //메서드 이름은 유지한 채 테스트이름만 변경
    void findByArticleId() {
        ...
        //Case 2: 1번 게시글의 댓글 조회
        {
            ...
            List<Comment> comments = commentRepository.findByArticleId(articleId);
            //3. 예상 데이터
            Article article = new Article(1L, "가가가", "11111");
            List<Comment> expected = Arrays.asList();
            //4. 비교 및 검증
            assertEquals(expected.toString(), comments.toString(),"1번 댓글이 없음");
        }
    }
